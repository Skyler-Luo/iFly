...
class FlightAPITest(APITestCase):
    ...
    def test_export_and_import_csv(self):
        ...
        f = io.StringIO()
        writer = csv.writer(f)
        writer.writerow(['flight_number','departure_city','arrival_city','departure_time','arrival_time','price','discount','capacity','available_seats','status','aircraft_type'])
        writer.writerow([self.flight.flight_number, 'A', 'B', (timezone.now()+timedelta(days=1)).isoformat(), (timezone.now()+timedelta(days=1)).isoformat(), '300', '1.0', '20', '20', 'scheduled', 'B747'])
